...
class ParagraphsContext extends RawDrupalContext {
  ...
  public function create($entity) {
    ...
    $saved = $this->drupalContext->getDriver()
      ->createEntity(self::ENTITY_TYPE_ID, $entity);
    $this->entities[] = $saved;
    return $saved;
  }
  ...
  public function cleanup() {
    ...
    foreach ($this->entities as $entity) {
      $this->drupalContext->getDriver()
        ->entityDelete(self::ENTITY_TYPE_ID, $entity);
    }
    $this->entities = [];
    $this->paragraphNames = [];
  }
   ...
   * | ...       | ...            |
   *
   * @Given a :type paragraph named :name
   */
